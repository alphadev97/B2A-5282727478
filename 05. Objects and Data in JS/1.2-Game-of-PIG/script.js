(function () {
  ...
  startGame.addEventListener("click", function () {
    ...
    gameControl.innerHTML += '<button id="quit">Do you want to quit?</button>'; // If you adding id in inner html use single quote around syntax
  ...
  function throwDice() {
    ...
    if (gameData.rollSum === 2) {
      game.innerHTML += "<p>Oh snap! You got snak eyes!</p>";
      ...
      gameData.index ? (gameData.index = 0) : (gameData.index = 1);
      game.innerHTML += `<p>You rolled a 1, switching to the ${
        gameData.players[gameData.index]
      }</p>`;
      setTimeout(setUpTurn, 2000);
    } else {
  ...
  function showCurrentScore() {
    score.innerHTML = `<p> the score for <strong> ${gameData.players[0]} is 
            ${gameData.score[0]} </strong> and the score for <strong> ${gameData.players[1]} 
